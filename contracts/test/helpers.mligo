...
type normalized_deposit = 
  address * address * (nat * bytes option) * nat
  ...
  let main (param : jstz) (storage : storage) : operation list * storage =
    match param with
      Deposit_fa_ticket { receiver; proxy; ticket } -> 
        let fa_deposit = normalized_fa_deposit receiver proxy ticket :: storage.fa_deposit in
...
let init_jstz_rollup () = 
  ...
  Test.Next.Originate.contract 
    (contract_of Mock_jstz_smart_rollup) 
    storage 
    0tez 
