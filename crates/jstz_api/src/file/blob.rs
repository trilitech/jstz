...
impl Blob {
    ...
    pub fn new(
        ...
        if blob_parts.is_none() && options.is_none() {
            return Ok(Self {
                bytes: vec![],
                type_: String::from(""),
                size: 0,
        ...
        let blob_parts = blob_parts.unwrap();
        ...
        if options.is_some()
            && options.as_ref().unwrap().type_.is_some()
            && !options.as_ref().unwrap().type_.as_ref().unwrap().is_empty()
        {
            t = normalize_type(
                options.as_ref().unwrap().type_.as_ref().unwrap().as_str(),
            );
        }
    ...
    pub fn slice(
        ...
        let original_size = i64::try_from(self.size).unwrap();
        ...
        let span = usize::try_from(max(relative_end - relative_start, 0)).unwrap();
        let relative_start = usize::try_from(relative_start).unwrap();
        ...
        //    b. S.size = span.
        let size = u64::try_from(span).unwrap();
...
impl TryFromJs for BlobPropertyBag {
    fn try_from_js(value: &JsValue, context: &mut Context<'_>) -> JsResult<Self> {
        ...
        let endings: Option<Endings> = if obj
            .has_property(js_string!("endings"), context)?
        {
            let endings =
                String::try_from_js(&obj.get(js_string!("endings"), context)?, context)?;
            let endings = match endings.as_str() {
