...
impl File {
    ...
    pub fn new(
        ...
        let blob = Blob::new(Some(file_bits), &blob_property_bag, context).unwrap();
...
impl TryFromJs for FilePropertyBag {
    fn try_from_js(value: &JsValue, context: &mut Context<'_>) -> JsResult<Self> {
        let blob_property_bag = BlobPropertyBag::try_from_js(value, context).unwrap();
        ...
        let last_modified: Option<i64> = if last_modified.is_object() {
            ...
            JsDate::from_object(obj.clone()).ok().map(|date| {
                f64::try_from_js(&date.get_time(context).unwrap(), context).unwrap()
                    as i64
            })
        } else if last_modified.is_number() {
            Some(i64::try_from_js(&last_modified, context)?)
        } else {
            None
        };
